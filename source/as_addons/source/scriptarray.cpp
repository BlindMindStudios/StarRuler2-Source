...
static CScriptArray* ScriptArrayListFactory(asITypeInfo *ot, void *initList)
{
...
static bool ScriptArrayTemplateCallback(asITypeInfo *ot, bool &dontGarbageCollect)
{
	...
	if( (typeId & asTYPEID_MASK_OBJECT) )
	{
		...
		else if( (flags & asOBJ_REF) && !(typeId & asTYPEID_OBJHANDLE) )
		{
...
static void RegisterScriptArray_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("array<T>", "T &get_last()", asMETHODPR(CScriptArray, Last, (), void*), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "const T &get_last() const", asMETHODPR(CScriptArray, Last, () const, const void*), asCALL_THISCALL); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("array<T>", "void remove(const T&in)", asMETHOD(CScriptArray, Remove), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("array<T>", "void removeAll(const T&in)", asMETHOD(CScriptArray, RemoveAll), asCALL_THISCALL); assert( r >= 0 );
...
{
	assert( delta < 1000000 && delta > -1000000  ); // Don't do this
	if( delta < 0 )
	{
...
void CScriptArray::Remove(void *value) {
	int found = Find(0, value);

	if( found >= 0 )
		Resize(-1, asUINT(found));
}

void CScriptArray::RemoveAll(void *value) {
	asUINT index = 0;

	do
	{
		int found = Find(index, value);
	...
	while( index < buffer->numElements );
}
...
const void *CScriptArray::Last() const
{
	if( buffer == 0 || buffer->numElements == 0)
	{
		...
		asIScriptContext *ctx = asGetActiveContext();
		if( ctx )
			ctx->SetException("Array is empty");
		return 0;
	}

	asUINT index = buffer->numElements - 1;
	if( (subTypeId & asTYPEID_MASK_OBJECT) && !(subTypeId & asTYPEID_OBJHANDLE) )
		return (void*)((size_t*)buffer->data)[index];
	else
		return buffer->data + elementSize*index;
...
{
	return const_cast<void*>(const_cast<const CScriptArray *>(this)->Last());
}
...
bool CScriptArray::Less(const void *a, const void *b, bool asc, asIScriptContext *ctx, SArrayCache *cache)
{
	...
	else
	{
		...
		if( cache && cache->cmpFunc )
		{
			...
			{
				int result = (int)ctx->GetReturnDWord() < 0;
				if(reversed)
					result *= -1;
				return result;
			}
...
bool CScriptArray::operator==(const CScriptArray &other) const
{
	...
	if( (subTypeId & ~asTYPEID_MASK_SEQNBR) && (cache && cache->eqFunc) )
	{
