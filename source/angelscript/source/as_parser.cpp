		...
		while( !isSyntaxError )
		{
			...
			if( t1.type == ttImport )
				...
				node->AddChildLast(ParseTypedef());		// Handle primitive typedefs
					...
					((IdentifierIs(t1, SHARED_TOKEN) || IdentifierIs(t1, FINAL_TOKEN) || IdentifierIs(t1, TIDY_TOKEN) || IdentifierIs(t1, ABSTRACT_TOKEN)) && t2.type == ttClass) || 
					 (IdentifierIs(t1, SHARED_TOKEN) && (IdentifierIs(t2, FINAL_TOKEN) || IdentifierIs(t2, ABSTRACT_TOKEN) || IdentifierIs(t2, TIDY_TOKEN))) ||
					 (IdentifierIs(t1, TIDY_TOKEN) && IdentifierIs(t2, FINAL_TOKEN)) ||
					 (IdentifierIs(t1, FINAL_TOKEN) && IdentifierIs(t2, TIDY_TOKEN))
					)
